## Read and follow the AGENTS.md strictly!

Create a "Subscription Payment" sits in the "Profile.tsx" under a new button called "Your subscription". 
The flow will be: tap the profile in the TabBar show the the Profile page > in the Profile page has a new created buton "Your Subscription". 

For now, leave the "Your Subscription" page empty and will implement the billing feature from Clark late (next prompt). The design of the "Your Subcription" button using the NativeWind design [glassmorphism- with  primary color for the font]

Below here is the instruction from the Clerk, after reading it and follow it > update the AGENTS.md with rules that follow the billing instruction.
--------------------------------------------------------------------------------------------------------------------------------------------------------
